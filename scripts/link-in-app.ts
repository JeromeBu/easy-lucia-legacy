...
//If you change the outDir in tsconfig.json you must update this block.
{
  ...
  modifiedPackageJsonContent = (() => {
    ...
    // biome-ignore lint/performance/noDelete: needed here
    delete o.files;
...
const execYarnLink = (params: { targetModuleName?: string; cwd: string }) => {
  ...
  console.info(`$ cd ${pathRelative(rootDirPath, cwd) || "."} && ${cmd}`);
...
console.info("=== Linking common dependencies ===");
...
commonThirdPartyDeps.forEach((commonThirdPartyDep) => {
  ...
  console.info(`${current}/${total} ${commonThirdPartyDep}`);
...
console.info("=== Linking in house dependencies ===");
